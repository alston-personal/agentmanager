...
ACTION_DEPLOY_REALM_FABRIC = "agentos.realm_fabric.deploy"
ALLOWED_ACTIONS = {
    ACTION_REPAIR_TRANSPORT,
    ACTION_DEPLOY_REALM_GATEWAY,
    ACTION_DEPLOY_REALM_FABRIC,
}
...
def _validate_request(path: Path, payload: dict[str, Any]) -> tuple[str, str, str | None]:
    ...
    if action in {ACTION_DEPLOY_REALM_GATEWAY, ACTION_DEPLOY_REALM_FABRIC} and source_commit is None:
...
def _execute(action: str, source_commit: str | None) -> dict[str, Any]:
    ...
    if action == ACTION_DEPLOY_REALM_FABRIC:
        return _run_canonical_script("scripts/deploy_realm_fabric_candidate_user.sh", timeout=180, source_commit=source_commit)
