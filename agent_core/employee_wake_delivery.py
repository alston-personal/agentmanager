...
def _attempt_id(wake_id: str, presence_generation: int, node_id: str) -> str:
    digest = hashlib.sha256(
        f"{wake_id}\0{presence_generation}\0{node_id}".encode("utf-8")
    ).hexdigest()[:24]
...
def _task_id(wake_id: str, presence_generation: int) -> str:
    digest = hashlib.sha256(
        f"{wake_id}\0{presence_generation}".encode("utf-8")
    ).hexdigest()[:24]
...
class EmployeeWakeDelivery:
    """Deliver bounded Employee wake hints through the existing ONE Node queue.

    The delivery is presence-generation scoped. A crash after the local
    `dispatching` record is persisted but before a terminal dispatch record is
    treated as UNKNOWN and is never blindly re-dispatched to the same presence.
    A later Employee presence generation may receive the same durable wake hint;
    assignment leasing remains the final double-execution fence.
    """

    def __init__(
        self,
        planner: EmployeeWakePlanner,
        presence: EmployeePresenceRegistry,
        controller: ControllerService,
    ) -> None:
        self.planner = planner
        self.presence = presence
        self.controller = controller
        self.root = planner.lifecycle.runtime.root / "realm" / "employee-wake-delivery"
    ...
    def deliver_next(
        ...
        *,
    ...
    ) -> EmployeeWakeDeliveryState | None:
        current = now or _utcnow()
        intent = self.planner.plan_next(employee_id, now=current)
        if intent is None:
            return None
        route = self.presence.resolve(employee_id, now=current)
        ...
        try:
            ...
            # ControllerService normally rejects before queueing, but storage or
            # process interruption around the queue boundary can be ambiguous.
            # Never serialize exception text or blindly retry the same presence.
            state.status = "unknown"
