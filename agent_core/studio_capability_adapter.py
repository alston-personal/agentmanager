    ...
    *,
    ...
    for key, raw in (data.get("capabilities") or {}).items():
        ...
        if not provider_id or not capability_uri:
            raise ValueError(f"Studio capability {key} is missing provider_id/capability_uri")
        if not provider_id.startswith("service://"):
            raise ValueError(f"Studio capability provider must be service://: {provider_id}")
        if not capability_uri.startswith("capability://"):
            raise ValueError(f"Studio capability URI must be capability://: {capability_uri}")
        ...
        entity = GovernanceEntity(
            ...
            implementation={
                ...
                "canonical_registry": str(source_path.relative_to(PROJECT_ROOT)),
