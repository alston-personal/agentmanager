...
marker_v3 = 'issue71_node_registry_ubuntu_preserve_v3'

if marker_v3 in text:
    print('issue71_repair_action_patch=ALREADY_PRESENT_V3')
    raise SystemExit(0)

# Upgrade a compiled v2 action: the original forensic directory belongs to the
# capture identity (agentos-node).  Keep that immutable evidence untouched and
# place the ubuntu-authorized supplemental pre-repair copy under ubuntu's own
# state directory instead.
if marker_v2 in text:
    old = """        stamp = _i71_datetime.now(_i71_timezone.utc).strftime('%Y%m%dT%H%M%SZ')
        preserve_dir = data_root / 'forensics/issue-71' / stamp
        preserve_dir.mkdir(parents=True, exist_ok=False)
"""
    new = """        # issue71_node_registry_ubuntu_preserve_v3
...
"""
    if old not in text:
        raise SystemExit('Issue 71 v2 preserve block not found')
    text = text.replace(old, new, 1)
    compile(text, str(path), 'exec')
    path.write_text(text, encoding='utf-8')
    print('issue71_repair_action_patch=UPGRADED_V3')
    raise SystemExit(0)

# Upgrade an already-compiled v1 action without broadening its authority. The
# original forensic copy is chmod 0600 by the agentos-node capture runner, so
# ubuntu cannot read that preserved file. The evidence chain is bridged by
# requiring the still-live malformed registry to have exactly the preserved
# forensic SHA256 before repair.
if marker in text:
...
"""
    ...
    text = text.replace(old, new, 1)
    preserve_old = """        stamp = _i71_datetime.now(_i71_timezone.utc).strftime('%Y%m%dT%H%M%SZ')
        preserve_dir = data_root / 'forensics/issue-71' / stamp
        preserve_dir.mkdir(parents=True, exist_ok=False)
"""
    preserve_new = """        # issue71_node_registry_ubuntu_preserve_v3
        stamp = _i71_datetime.now(_i71_timezone.utc).strftime('%Y%m%dT%H%M%SZ')
        preserve_dir = Path.home() / '.local/state/agentos/forensics/issue-71' / stamp
        preserve_dir.mkdir(parents=True, exist_ok=False)
"""
    if preserve_old not in text:
        raise SystemExit('Issue 71 v1 preserve block not found')
    text = text.replace(preserve_old, preserve_new, 1)
    compile(text, str(path), 'exec')
    path.write_text(text, encoding='utf-8')
    print('issue71_repair_action_patch=UPGRADED_V3')
    raise SystemExit(0)
...
def _issue71_repair_node_registry(params: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            'forensic_bridge': 'live_sha_equals_preserved_forensic_sha',
...
compile(text, str(path), 'exec')
path.write_text(text, encoding='utf-8')
print('issue71_repair_action_patch=PASS_V3')
