...
if command -v gh >/dev/null 2>&1; then
  if env -u GH_TOKEN -u GITHUB_TOKEN gh api "repos/$REPOSITORY/issues/$ISSUE" --jq '.number' >/dev/null 2>&1; then
    ...
    if validate_token "$CANDIDATE"; then TOKEN="$CANDIDATE"; SOURCE='gh-auth'; fi
  fi
fi
if [ -z "$TOKEN" ]; then
  CREDENTIAL=$(printf 'protocol=https\nhost=github.com\n\n' | git credential fill 2>/dev/null || true)
  CANDIDATE=$(printf '%s\n' "$CREDENTIAL" | sed -n 's/^password=//p' | head -n 1)
  if validate_token "$CANDIDATE"; then TOKEN="$CANDIDATE"; SOURCE='git-credential'; fi
fi
if [ -z "$TOKEN" ]; then
...
# Exactly eleven fixed KEY=VALUE lines; arbitrary legacy environment is never preserved.
[ "$(wc -l < "$TMP")" -eq 11 ] || { echo "ERROR: rebuilt env line count invalid" >&2; exit 23; }
...
grep -Fq "AGENTOS_CONTROL_ALLOWED_ACTIONS=$ALLOWED_ACTIONS" "$ENV_FILE"
