...
OUT = ROOT / '.agentos/evidence/studio-web-remote-action-relay.txt'
ACTION = 'github.repo.ensure_studio_web'
...
payload=client.submit(ACTION, {'repository':'alston-personal/studio-web'})
...
lines.append('studio_web_remote_action_relay=' + ('PASS' if receipt.get('ok') is True else 'NO_GO'))
OUT.write_text('\n'.join(lines)+'\n',encoding='utf-8')
