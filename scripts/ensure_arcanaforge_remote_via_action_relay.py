...
OUT = ROOT / '.agentos/evidence/arcanaforge-remote-action-relay.txt'
ACTION = 'github.repo.ensure_arcanaforge'
REPOSITORY = 'alston-personal/arcanaforge'
...
lines.append('arcanaforge_remote_action_relay=' + ('PASS' if receipt.get('ok') is True else 'NO_GO'))
OUT.write_text('\n'.join(lines)+'\n',encoding='utf-8')
