...
def main() -> int:
    try:
        ...
        if isinstance(exc, subprocess.CalledProcessError):
            ...
            error_code = ('git_ownership_mismatch' if b'dubious ownership' in stderr
                          else 'git_read_failed')
        elif isinstance(exc, subprocess.TimeoutExpired):
            error_code = 'git_read_timeout'
        elif isinstance(exc, ValueError) and str(exc) in (
                'repository identity mismatch', 'invalid head', 'no caller parameters'):
            error_code = str(exc).replace(' ', '_')
        print(json.dumps({'schema': 'agentos.checkout-diagnostic/v1',
                          'classification': 'diagnostic_unavailable', 'error_code': error_code,
