...
| --- | --- | --- | --- |
| Vendor Reputation | `alston-personal/vendor-reputation-service` | PR #97 patrol artifact carrier, #99 calibration carrier, #127 monitored-source scheduler | Keep temporarily as migration sources; do not merge into Core. Canonical product handoff: `vendor-reputation-service#27`. Product implementation/workload/schedule belongs to Vendor; Core exposes only governed Oracle execution primitives. |
| ArcanaForge | `alston-personal/arcanaforge` | PR #136 POC release carrier | Keep as migration source until #66 provides a governed static-release path and exact source/artifact receipt. Canonical product handoff: `arcanaforge#3`. |
| LayoutLib | `alston-personal/layoutlib` | Core-side deploy/governance residue tracked by #96 | Keep only generic deployment/promotion adapter. Canonical product handoff: `layoutlib#2`. Layout implementation/tests stay in `layoutlib`; POC consumes exact candidate SHA, production consumes promoted release. |
...
Each of #97, #99, #127, #136, #53, and #125 has an explicit freeze/migration comment. Freeze means no direct merge into `agentmanager`; it does not mean historical branch/evidence may be deleted.
...
Vendor-owned scope, tracked by `vendor-reputation-service#27`:
...
PR #18 is a generic social-capability candidate and should be reviewed as a capability boundary. PR #125 is the mixed candidate described above and remains frozen until #144 and Vendor migration account for both ownership domains.
...
Migration does not globally pause a product. Only the exact step requiring an unavailable Core capability is blocked. Product feature work continues in its canonical repository while migration/deployment dependencies are resolved.
