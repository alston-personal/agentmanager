...
class TestRealmResolveEndpoint(unittest.TestCase):
    ...
    def test_enrolled_node_can_resolve_through_one(self):
        ...
        with patch("agent_core.realm_server.bootstrap_snapshot", return_value={"node_id": "chatgpt-web-test"}), patch(
            "agent_core.realm_server.resolve_continuation", return_value=expected
        ) as resolver:
            status, payload = self._post(
                {
                    "schema": "agentos.resolve-request/v1",
