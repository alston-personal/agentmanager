...
def test_public_trial_does_not_expose_hidden_key_fields():
    trial, key = build_blind_trial(73129)
    payload = public_json(trial)
    assert key.authority_boundary_step not in payload or "authority_boundary_step" not in payload
...
def test_short_trials_are_rejected():
    try:
        build_blind_trial(1, material_actions=19)
