...
## 🧮 0.1.1 Token 節流協議 (Token Efficiency Protocol)
> [!IMPORTANT]
> **輸出、搜尋、讀檔都必須受 token 預算約束。**
> 1. **先窄後寬**：先用最小路徑、最小檔案集、最小時間範圍搜尋；只有證據不足時才擴大。
> 2. **分段讀取**：預設以 80-220 行為單位讀檔，只有前一段證明相關才繼續往下讀。
> 3. **只報高訊號**：回覆優先提供結論、證據、下一步；預設只列 top 3-5 項發現，不做無上限枚舉。
> 4. **禁止預設傾倒**：除非使用者明確要求，禁止貼出完整檔案、超長 diff、整段 log、完整 stack trace。
> 5. **大型任務分期**：全 repo 掃描、遷移、review、盤點類任務，必須拆成可回報的 phase，逐段完成。
> 6. **避免重述**：架構背景、已知結論、先前發現不得在同一回合內反覆重寫；改用摘要或引用。
> 7. **長上下文先壓縮**：當對話或工作記錄變長時，只保留決策、blocker、風險、下一步，其餘丟棄。
> 8. **詳細規定**：參考 `.agent/rules/TOKEN_EFFICIENCY_PROTOCOL.md`。
...
### 🔹 Token 友善讀取 (Token-Friendly Inspection)
- 先用 `rg` / `rg --files` 縮小範圍，再讀檔。
- 大檔案優先用 `sed -n`、`head`、`tail`、`rg -n` 擷取片段，不直接整份輸出。
- 指令輸出過大時，改收斂查詢條件重跑，不以更長輸出硬吃。
